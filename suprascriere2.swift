...
enum test_cases: String{
    ...
    case reuniune_2 = "5"
    case intersectie = "6"
    case overwrite_plus_egal = "7"
    case overwrite_plus = "8"
    case contains = "9"
    case subset = "10"
    case egal = "11"
}

var test = test_cases.egal
...
switch test.rawValue{
    ...
    case "7": // testam overwriting de +=
        let a = t_set_of_primes(7,41)
        a += 5
        a << 43
        a.print_set()
    
    case "8": // testam overwriting de +
        let m = t_set_of_primes(7,41)
        let x = t_set_of_primes(30,100)
    ...
    case "11":
        let m = t_set_of_primes(30,100)
        let x = t_set_of_primes(30,100)
        if m == x{
            print("sunt identice")
        }else{
            print("nu sunt identice")
        }
